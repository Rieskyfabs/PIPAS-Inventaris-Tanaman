<section class="home relative mt-0 pt-0 bg-cover bg-center bg-no-repeat pb-12" id="home" style="background-image: url('/images/Bg-Hero.png');">
    <div class="container mx-auto flex flex-wrap justify-between items-center relative z-10 pb-24">
        <!-- Left Content -->
        <div class="content-left w-full lg:w-1/2 px-5 py-5">
            <h1 class="heading text-4xl lg:text-[4.1rem] font-bold text-white mt-40 leading-tight">
                Tingkatkan <span class="text-[#009379]">Kemampuan</span> mu Dengan <span class="text-[#009379]">PIPAS</span>
            </h1>
            <p class="subHeading text-lg lg:text-[1.313rem] font-light text-white mt-16 mb-12 leading-relaxed">
                Bakar semangatmu dengan mengikuti <span class="font-medium text-[#009379]">Pendidikan Ilmu Pengetahuan Alam dan Sosial</span> yang diadakan di sekolah sebagai ajang bagi dirimu untuk meningkatkan nilai dan sikap bagi para siswa.
            </p>
            <div class="box-wrapper flex flex-wrap gap-5">
                <div class="box flex items-center gap-4">
                    @include('landing-page.components._auth-button')
                </div>
                <div class="box flex items-center gap-4">
                    <a href="#about" class="btn-about flex items-center text-[#009379] font-semibold bg-[#F8F9FF] px-6 py-3 rounded-full border-2 border-[#009379] transition duration-300 hover:bg-[#009379] hover:text-[#F8F9FF]">
                        <i class="fas fa-info-circle mr-2"></i> Tentang PIPAS
                    </a>
                </div>
            </div>
        </div>
        
        <!-- Right Content -->
        <div class="content-right w-full lg:w-1/2 flex justify-center lg:justify-end">
            <div class="img-wrapper max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg relative lg:left-0 mt-20 lg:mt-44">
                <img src="{{ asset('/images/Hero-tanaman-IMG.png') }}" alt="" class="w-full h-auto">
            </div>
        </div>

    </div>
</section>

{{-- <section class="home" id="home">
    <div class="container home-wrapper">
        <div class="content-left">
            <h1 class="heading">Tingkatkan <span>Kemampuan</span> mu Dengan <span>PIPAS</span></h1>
            <p class="subHeading">Bakar semangatmu dengan mengikuti <span>Pendidikan Ilmu Pengetahuan Alam dan Sosial</span> yang diadakan di sekolah sebagai ajang bagi dirimu untuk meningkatkan nilai dan sikap bagi para siswa.</p>
            <div class="box-wrapper">
                <div class="box">
                    @include('landing-page.components._auth-button')
                </div>
                <div class="box">
                    <a href="#about" class="btn-about"><i class="fas fa-info-circle"></i> Tentang PIPAS</a>
                </div>
            </div>
        </div>
        <div class="content-right">
            <div class="img-wrapper">
                <img src="{{ asset('/images/Hero-tanaman-IMG.png') }}" alt="">
            </div>
        </div>
    </div>
</section> --}}